...
function HelpSupport() {
  ...
  return (
    <div className="hs-container">
      <div className="hs-wrapper">
        ...
        <div className="hs-header">
          <div className="hs-header-content">
            <button onClick={() => navigate(-1)} className="hs-back-button">
              <FaArrowLeft />
              <span>Back</span>
            </button>
            <div className="hs-header-title">
              <FaHeadphones className="hs-header-icon" />
              <h1>Help & Support</h1>
            </div>
          </div>
        </div>
        ...
        <div className="hs-hero">
          <div className="hs-hero-content">
            <div className="hs-hero-badge">
              <FaStar className="hs-badge-icon" />
            ...
            <div className="hs-search-container">
              ...
              />
              {searchTerm && (
                <button onClick={() => setSearchTerm('')} className="hs-clear-search">
                  <FaTimes />
                </button>
              )}
            </div>
          </div>
          <div className="hs-hero-decoration"></div>
        </div>
        ...
        <div className="hs-quick-actions">
          ...
          <div className="hs-actions-grid">
            {quickActions.map(action => (
              ...
              >
                <div className="hs-action-icon">
                  <action.icon />
                </div>
                <div className="hs-action-content">
                  <h4>{action.title}</h4>
                  <p>Get immediate assistance</p>
                </div>
                <FaChevronDown className="hs-action-arrow" />
              </button>
            ))}
          </div>
        </div>
        ...
        {/* Support Channels */}
        <div className="hs-support-channels">
          <br/>
          <h3>Support Channels</h3>
          <br/>
          <div className="hs-channels-grid">
            <button onClick={() => setShowChat(true)} className="hs-channel-card hs-channel-live" aria-label="Live Chat Support">
              <div className="hs-channel-icon">
                <FaCommentDots />
              </div>
              <h4>Live Chat</h4>
              <p>Chat with our support team</p>
              <span className="hs-channel-status">Available 24/7</span>
            </button>
            
            <button onClick={() => setShowContactForm(true)} className="hs-channel-card hs-channel-email" aria-label="Email Support">
              <div className="hs-channel-icon">
                <FaEnvelope />
              </div>
              <h4>Email Support</h4>
              <p>Get detailed responses</p>
              <span className="hs-channel-status">Response within 24h</span>
            </button>
            
            <button onClick={() => toast.info('Call us at +1 (555) 123-4567')} className="hs-channel-card hs-channel-phone" aria-label="Phone Support">
              <div className="hs-channel-icon">
                <FaPhone />
              </div>
              <h4>Phone Support</h4>
              <p>Speak to a representative</p>
              <span className="hs-channel-status">Mon-Fri, 9AM-6PM</span>
            </button>
          </div>
        </div>
        ...
        <div className="hs-faq-section">
          ...
          <div className="hs-faq-grid">
            {filteredFAQs.map((category, catIndex) => (
              <div key={catIndex} className="hs-faq-category">
                <button
                  ...
                  aria-controls={`hs-faq-cat-${catIndex}`}
                  tabIndex={0}
                >
                  <div className="hs-category-title">
                    <category.icon className="hs-category-icon" />
                ...
                <div
                  className={`hs-category-questions ${activeCategory === catIndex ? 'hs-active' : ''}`}
                  id={`hs-faq-cat-${catIndex}`}
                  aria-hidden={activeCategory !== catIndex}
                >
                  {category.questions.map((item, qIndex) => (
                    <div key={qIndex} className="hs-question-item">
                      <div className="hs-question">
                        <FaQuestionCircle className="hs-question-icon" />
                        <p>{item.q}</p>
                      </div>
        ...
        {/* Still Need Help */}
        <div className="hs-still-need-help">
          <div className="hs-help-card">
            <FaRobot className="hs-help-icon" />
            <h3>Still need help?</h3>
            <p>Our support team is ready to assist you with any issues</p>
            <button onClick={() => setShowContactForm(true)} className="hs-contact-button" aria-label="Contact Support">
              Contact Support
              <FaPaperPlane />
            </button>
          </div>
        ...
        <div className="hs-social-links">
          ...
          <div className="hs-social-icons">
            <a href="#" className="hs-social-icon hs-social-twitter">
              <FaTwitter />
            </a>
            <a href="#" className="hs-social-icon hs-social-facebook">
              <FaFacebook />
            </a>
            <a href="#" className="hs-social-icon hs-social-instagram">
              <FaInstagram />
            </a>
          </div>
        </div>
      ...
      {/* Contact Form Modal */}
      {showContactForm && (
        <div className="hs-modal-overlay" onClick={() => setShowContactForm(false)}>
          <div className="hs-modal-content" onClick={e => e.stopPropagation()}>
            <div className="hs-modal-header">
              <h3>Contact Support</h3>
              <button onClick={() => setShowContactForm(false)} className="hs-modal-close" aria-label="Close Contact Form">
                <FaTimes />
              </button>
            </div>
            <form onSubmit={handleSubmitContact} className="hs-contact-form">
              ...
              <button type="submit" className="hs-submit-button" aria-label="Send Message">
                <FaPaperPlane />
                Send Message
              </button>
            </form>
        ...
        <div className="hs-chat-modal">
          <div className="hs-chat-container">
            <div className="hs-chat-header">
              ...
              <button onClick={() => setShowChat(false)} className="hs-chat-close" aria-label="Close Chat">
                <FaTimes />
              </button>
            </div>
            <div className="hs-chat-messages">
              {chatMessages.map((msg, idx) => (
                <div key={idx} className={`hs-chat-message hs-chat-${msg.type}`}>
                  <div className="hs-message-bubble">
                    <p>{msg.message}</p>
                    <span className="hs-message-time">{msg.time}</span>
                  </div>
                </div>
              ))}
            ...
            <div className="hs-chat-input-container">
              <input
                type="text"
                value={chatInput}
              ...
              />
              <button onClick={handleSendChat} className="hs-chat-send" aria-label="Send Chat Message">
                <FaPaperPlane />
              </button>
            </div>
          </div>
